    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-[var(--text-muted)] mt-0.5">Manage enterprise company announcements and system notifications</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[var(--bg-card)] border border-[var(--border-base)] rounded-2xl p-5 shadow-lg flex flex-col justify-between">
          <div>
            ...
            {loading ? (
              <div className="py-8 text-center text-xs text-[var(--text-muted)]">Loading broadcasts...</div>
            ) : announcements.length === 0 ? (
              <div className="py-8 text-center text-xs text-[var(--text-muted)]">No company announcements published yet.</div>
            ) : (
              <div className="space-y-3">
                {announcements.map((ann) => (
                  <div key={ann.id} className="bg-[var(--bg-card-alt)] border border-[var(--border-base)] rounded-xl p-4 relative group">
                    <div className="flex justify-between items-start">
                      <h4 className="text-xs font-bold text-white">{ann.title}</h4>
                      <button
                        onClick={() => handleDeleteAnnouncement(ann.id)}
                        className="text-[var(--text-muted)] hover:text-red-400 opacity-0 group-hover:opacity-100 transition p-1"
                      >
                        <Trash2 className="w-3.5 h-3.5" />
                      </button>
                    </div>
                    <p className="text-xs text-[var(--text-secondary)] mt-1 leading-relaxed">{ann.body}</p>
        ...
        <div className="bg-[var(--bg-card)] border border-[var(--border-base)] rounded-2xl p-5 shadow-lg flex flex-col justify-between">
          <div>
            ...
            {loading ? (
              <div className="py-8 text-center text-xs text-[var(--text-muted)]">Loading notifications...</div>
            ) : notifications.length === 0 ? (
              <div className="py-8 text-center text-xs text-[var(--text-muted)]">Your notifications inbox is clean!</div>
            ) : (
              <div className="space-y-3">
                {notifications.map((notif) => (
                  <div
                    ...
                    className={`border rounded-xl p-4 flex items-start justify-between gap-3 transition ${
                      notif.read ? 'bg-[var(--bg-card-alt)]/30 border-[var(--border-base)] opacity-60' : 'bg-[var(--bg-card-alt)]/80 border-[var(--border-muted)]'
                    }`}
                  >
                    <div>
                      <h4 className="text-xs font-bold text-white">{notif.title}</h4>
                      <p className="text-xs text-[var(--text-secondary)] mt-1 leading-relaxed">{notif.message}</p>
                      <span className="text-[10px] text-[var(--text-muted)] mt-2 block">{new Date(notif.createdAt).toLocaleString()}</span>
                    </div>
                    {!notif.read && (
                      <button
                        onClick={() => handleMarkRead(notif.id)}
                        className="p-1 bg-gray-800 hover:bg-gray-700 text-[var(--text-primary)] rounded text-[10px] font-bold uppercase shrink-0"
                        title="Mark as Read"
                      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md bg-[var(--bg-card)] border border-[var(--border-muted)] rounded-2xl p-6 shadow-2xl">
            <div className="flex justify-between items-center mb-5">
              ...
              <button onClick={() => setShowAnnounceModal(false)} className="text-[var(--text-secondary)] hover:text-white"><X className="w-5 h-5" /></button>
            </div>
            <form onSubmit={handleCreateAnnouncement} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1 uppercase">Announcement Title</label>
                <input
                  required
              ...
              <div>
                <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1 uppercase">Target Audience</label>
                <select
              ...
              <div>
                <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1 uppercase">Announcement Message Content</label>
                <textarea
                  required
                  rows={4}
              ...
              <div className="flex gap-3 pt-3">
                <button type="button" onClick={() => setShowAnnounceModal(false)} className="flex-1 py-2 border border-[var(--border-muted)] text-[var(--text-primary)] text-sm rounded-lg">Cancel</button>

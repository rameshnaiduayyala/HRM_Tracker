    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Assign tasks, track work status, and log employee task completion</p>
        </div>
        {!employeeId && (
          <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-[#111827] border border-gray-800 rounded-xl p-4 flex flex-col justify-between">
          <span className="text-[10px] uppercase font-bold text-gray-400 tracking-wider">Total Tasks</span>
        ...
        <div className="bg-[#111827] border border-gray-800 rounded-xl p-4">
          <div className="flex justify-between items-baseline">
            ...
            <span className="text-xs text-gray-400 font-bold">{completionRate}%</span>
          </div>
      ...
      {!employeeId && (
        <div className="bg-[#111827] border border-gray-800 rounded-xl p-4 flex flex-wrap gap-4 items-center">
          <div className="flex flex-col gap-1 flex-1 min-w-[150px]">
            <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Assignee Filter</label>
            <select
              value={filterAssignee}
              onChange={(e) => setFilterAssignee(e.target.value)}
              className="bg-gray-950 border border-gray-800 text-xs text-gray-300 rounded-lg px-3 py-2 outline-none cursor-pointer"
            >
          ...
          <div className="flex flex-col gap-1 flex-1 min-w-[150px]">
            <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Project Board</label>
            <select
              value={filterProject}
          ...
          <button
            ...
            className="self-end px-4 py-2 border border-gray-800 hover:border-gray-700 text-xs font-semibold text-gray-400 hover:text-white rounded-lg transition"
          >
            Clear Filters
          </button>
        </div>
      ...
      {/* Kanban Board Layout */}
      {loading ? (
        <div className="py-12 text-center text-xs text-gray-500">Loading task boards...</div>
      ) : (
            ...
            return (
              <div key={col.id} className={`border rounded-2xl p-4 flex flex-col min-h-[500px] ${col.color}`}>
                ...
                <div className="space-y-3 flex-1 overflow-y-auto">
                  {colTasks.length === 0 ? (
                    <div className="py-8 text-center text-[11px] text-gray-600 italic">No tasks here</div>
                  ) : (
                    colTasks.map((t) => {
                      ...
                      return (
                        <div
                          ...
                          className="bg-[#111827] border border-gray-800 hover:border-gray-700 rounded-xl p-3 shadow-md cursor-pointer transition group"
                        >
                          ...
                          {t.description && <p className="text-[11px] text-gray-400 mt-1 line-clamp-2">{t.description}</p>}

                          <div className="mt-3 pt-2 border-t border-gray-850 flex items-center justify-between text-[10px] text-gray-500">
                            {assignee ? (
                            ...
                            <select
                              ...
                              className="bg-gray-900 border border-gray-800 text-[9px] text-gray-300 rounded px-1 py-0.5"
                            >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md bg-[#111827] border border-gray-700 rounded-2xl p-6 shadow-2xl">
            <div className="flex justify-between items-center mb-5">
              ...
              <button onClick={() => setShowTaskModal(false)} className="text-gray-400 hover:text-white"><X className="w-5 h-5" /></button>
            </div>
            <form onSubmit={handleCreateTask} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1 uppercase">Task Title</label>
                <input
                  required
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1 uppercase">Project Board</label>
                <select
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-gray-400 mb-1 uppercase">Assignee</label>
                  <select
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-400 mb-1 uppercase">Priority</label>
                  <select
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1 uppercase">Due Date</label>
                <input
                  type="date"
              ...
              <div className="flex gap-3 pt-3">
                <button type="button" onClick={() => setShowTaskModal(false)} className="flex-1 py-2 border border-gray-700 text-gray-300 text-sm rounded-lg">Cancel</button>
      ...
      {selectedTask && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-lg bg-[#111827] border border-gray-700 rounded-2xl p-6 shadow-2xl space-y-4">
            <div className="flex justify-between items-start">
              ...
              <button onClick={() => setSelectedTask(null)} className="text-gray-400 hover:text-white"><X className="w-5 h-5" /></button>
            </div>
            ...
            <div className="space-y-3 pt-2">
              ...
              <div className="max-h-40 overflow-y-auto space-y-2 pr-1">
                {(selectedTask.comments || []).length === 0 ? (
                  <p className="text-[11px] text-gray-500 italic">No comments logged yet.</p>
                ) : (
                  (selectedTask.comments || []).map((c, i) => (
                    <div key={i} className="bg-gray-900 p-2.5 rounded-lg border border-gray-800 text-xs">
                      <p className="text-gray-300">{c.content}</p>
                      <span className="text-[9px] text-gray-500 mt-1 block">{new Date(c.createdAt).toLocaleString()}</span>
                    </div>
                  ))
                )}
              </div>
              ...
              <form onSubmit={handleAddComment} className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-1.5 bg-gray-900 border border-gray-700 text-white text-xs rounded-lg"
                />

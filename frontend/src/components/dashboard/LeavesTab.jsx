    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Manage employee leave balances, approval workflows, and time-off types</p>
        </div>
      ...
      <div className="flex border-b border-gray-800 gap-4">
        <button
          ...
          className={`pb-3 text-xs font-bold uppercase tracking-wider border-b-2 transition ${
            activeTab === 'requests' ? 'border-indigo-500 text-white' : 'border-transparent text-gray-400 hover:text-white'
          }`}
        >
        ...
        <button
          ...
          className={`pb-3 text-xs font-bold uppercase tracking-wider border-b-2 transition ${
            activeTab === 'policies' ? 'border-indigo-500 text-white' : 'border-transparent text-gray-400 hover:text-white'
          }`}
        >
        ...
        <button
          ...
          className={`pb-3 text-xs font-bold uppercase tracking-wider border-b-2 transition ${
            activeTab === 'holidays' ? 'border-indigo-500 text-white' : 'border-transparent text-gray-400 hover:text-white'
          }`}
        >
        ...
        <button
          ...
          className={`pb-3 text-xs font-bold uppercase tracking-wider border-b-2 transition ${
            activeTab === 'balances' ? 'border-indigo-500 text-white' : 'border-transparent text-gray-400 hover:text-white'
          }`}
        >
          Accruals & Balances
        </button>
      ...
      {activeTab === 'requests' && (
        <div className="bg-[#111827] border border-gray-800 rounded-2xl overflow-hidden shadow-lg">
          {loading ? (
            <div className="p-8 text-center text-gray-500 text-xs">Loading leave requests...</div>
          ) : requests.length === 0 ? (
            <div className="p-12 text-center text-gray-500 text-xs">No leave applications recorded yet.</div>
          ) : (
              ...
              <table className="w-full text-left text-xs text-gray-300">
                ...
                <tbody className="divide-y divide-gray-800/60">
                  {requests.map((req) => {
                    ...
                    return (
                      <tr key={req.id} className="hover:bg-gray-900/40 transition">
                        <td className="px-6 py-4 font-medium text-white">{empName}</td>
                        <td className="px-6 py-4">
                          <span className="font-semibold text-gray-300">{lt?.name || 'Leave'}</span>
                        </td>
                        <td className="px-6 py-4 text-gray-400">
                          {new Date(req.startDate).toLocaleDateString()} - {new Date(req.endDate).toLocaleDateString()}
                        </td>
                        <td className="px-6 py-4 text-gray-400 max-w-xs truncate">{req.reason || 'N/A'}</td>
                        <td className="px-6 py-4 text-gray-500 italic max-w-xs truncate">{req.reviewNote || '—'}</td>
                        <td className="px-6 py-4">
                          <span
                        ...
                        <td className="px-6 py-4 text-right space-x-2">
                          ...
                          <button
                            ...
                            className="p-1.5 bg-gray-800 hover:bg-red-950 text-gray-500 hover:text-red-400 rounded-lg border border-gray-700 hover:border-red-800/40 transition"
      ...
      {activeTab === 'policies' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {leaveTypes.map((lt) => (
            <div key={lt.id} className="bg-[#111827] border border-gray-800 rounded-2xl p-5 flex flex-col justify-between group relative shadow-md">
              ...
              <div className="mt-6 flex items-center justify-between text-xs text-gray-400">
                ...
                <span className={`px-2.5 py-0.5 rounded-md text-[10px] font-bold ${lt.isPaid ? 'bg-emerald-950 text-emerald-400 border border-emerald-800/30' : 'bg-gray-800 text-gray-400'}`}>
      ...
      {activeTab === 'holidays' && (
        <div className="bg-[#111827] border border-gray-800 rounded-2xl overflow-hidden">
          ...
          {holidays.length === 0 ? (
            <p className="p-8 text-center text-gray-500 text-xs">No standard public holidays registered yet.</p>
          ) : (
            <div className="divide-y divide-gray-800/60">
              {holidays.map((h) => (
                <div key={h.id} className="px-6 py-4 flex items-center justify-between hover:bg-gray-900/30 transition">
                  <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="p-2 bg-gray-800 hover:bg-red-950/40 text-gray-400 hover:text-red-400 rounded-xl transition border border-gray-700 hover:border-red-950"
                  >
                    <Trash2 className="w-3.5 h-3.5" />
                  </button>
                </div>
      ...
      {activeTab === 'balances' && (
        <div className="space-y-4">
          <div className="bg-[#111827] border border-gray-800 rounded-2xl p-5 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <BookOpen className="w-5 h-5 text-indigo-400" />
              <div>
                <h3 className="text-sm font-bold text-white">Employee Accrual Ledger</h3>
                <p className="text-xs text-gray-500">Select employee to display current available leave balances</p>
              </div>
            </div>
            <select
              onChange={(e) => fetchEmployeeBalances(e.target.value)}
              className="px-3 py-1.5 bg-gray-900 border border-gray-700 text-white text-xs rounded-xl focus:outline-none"
            >
              {employees.map(emp => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {balances.length === 0 ? (
              <div className="md:col-span-3 py-12 text-center text-gray-500 text-xs">No active leave type balances tracked for this employee.</div>
            ) : (
              balances.map((bal) => (
                <div key={bal.id} className="bg-[#111827] border border-gray-800 rounded-2xl p-5 shadow-sm">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md bg-[#111827] border border-gray-700 rounded-2xl p-6 shadow-2xl">
            <div className="flex justify-between items-center mb-5">
              ...
              <button onClick={() => setShowTypeModal(false)} className="text-gray-400 hover:text-white"><X className="w-5 h-5" /></button>
            </div>
            <form onSubmit={handleCreateOrUpdateType} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1 uppercase">Leave Policy Name</label>
                <input
                  required
                  value={typeForm.name}
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1 uppercase">Allocated Days / Year</label>
                <input
                  type="number"
                  required
                  min="1"
              ...
              <div className="flex items-center gap-2 pt-1">
                <input
                  ...
                  className="rounded border-gray-700 text-indigo-600 bg-gray-900"
                />
                <label htmlFor="paidCheck" className="text-xs text-gray-300">Paid Leave Type</label>
              </div>
              <div className="flex gap-3 pt-3">
                <button type="button" onClick={() => setShowTypeModal(false)} className="flex-1 py-2 border border-gray-700 text-gray-300 text-sm rounded-lg">Cancel</button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md bg-[#111827] border border-gray-700 rounded-2xl p-6 shadow-2xl">
            <div className="flex justify-between items-center mb-5">
              ...
              <button onClick={() => setShowRequestModal(false)} className="text-gray-400 hover:text-white"><X className="w-5 h-5" /></button>
            </div>
            <form onSubmit={handleCreateRequest} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1 uppercase">Leave Applicant</label>
                {currentUserEmployee ? (
                  <input
                    type="text"
                    readOnly
                    value={`${user.firstName} ${user.lastName}`}
                    className="w-full px-3 py-2 bg-gray-800 border border-gray-700 text-gray-400 text-sm rounded-lg cursor-not-allowed"
                  />
                ) : (
                  <select
                    required
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1 uppercase">Leave Policy Category</label>
                <select
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-400 mb-1 uppercase">End Date</label>
                  <input
                    type="date"
                    required
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1 uppercase">Reason</label>
                <textarea
                  rows={3}
              ...
              <div className="flex gap-3 pt-3">
                <button type="button" onClick={() => setShowRequestModal(false)} className="flex-1 py-2 border border-gray-700 text-gray-300 text-sm rounded-lg">Cancel</button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-sm bg-[#111827] border border-gray-700 rounded-2xl p-6 shadow-2xl">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setShowReviewModal(null)} className="text-gray-400 hover:text-white"><X className="w-5 h-5" /></button>
            </div>
            <form onSubmit={handleReviewSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1.5 uppercase">Review Note / Decision Reason</label>
                <textarea
                  rows={3}
                  value={reviewNote}
                  onChange={(e) => setReviewNote(e.target.value)}
                  placeholder="Optional review reason (e.g., covered by team members)..."
                  className="w-full px-3 py-2 bg-gray-900 border border-gray-700 text-white text-sm rounded-lg resize-none focus:border-indigo-500 focus:outline-none"
                />
              </div>
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 py-2 border border-gray-700 text-gray-300 text-xs rounded-lg"
                >
                  Cancel
                </button>
                <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-sm bg-[#111827] border border-gray-700 rounded-2xl p-6 shadow-2xl">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setShowHolidayModal(false)} className="text-gray-400 hover:text-white"><X className="w-5 h-5" /></button>
            </div>
            <form onSubmit={handleCreateHoliday} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1 uppercase">Holiday Name</label>
                <input
                  required
              ...
              <div className="flex gap-2 pt-2">
                <button type="button" onClick={() => setShowHolidayModal(false)} className="flex-1 py-2 border border-gray-700 text-gray-300 text-xs rounded-lg">Cancel</button>

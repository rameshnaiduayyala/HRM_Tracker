...
<body>
  ...
  <div class="footer">
    This document serves as an official Relieving Certificate generated by ${params.companyName}.
  </div>
</body>
</html>
  `;
...
export function generateExperienceLetterHTML(params: RelievingLetterParams): string {
  return `
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <title>Experience Certificate - ${params.employeeName}</title>
  <style>
    body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; margin: 40px; color: #1e293b; background: #fff; }
    ...
    .company-name { font-size: 24px; font-weight: bold; color: #4338ca; }
    .doc-title { font-size: 22px; font-weight: 800; text-transform: uppercase; letter-spacing: 1.5px; color: #3730a3; text-align: center; margin: 24px 0; border-bottom: 1px solid #e0e7ff; padding-bottom: 12px; }
    .content { font-size: 14.5px; line-height: 1.85; color: #334155; }
    .highlight { font-weight: 700; color: #4338ca; }
    .badge-box { background: #f5f3ff; border: 1px solid #ddd6fe; border-radius: 8px; padding: 14px 18px; margin: 20px 0; font-size: 13.5px; }
    .footer { margin-top: 60px; font-size: 12.5px; color: #64748b; border-top: 1px solid #e2e8f0; padding-top: 20px; }
    .signature-box { width: 220px; border-top: 1.5px solid #6366f1; text-align: center; padding-top: 8px; font-weight: 700; margin-top: 65px; color: #1e1b4b; }
    @media print {
      body { margin: 0; padding: 20px; }
      .no-print { display: none; }
    }
...
<body>
  ...
  <div class="doc-title">Work Experience Certificate</div>

  <div class="content">
    <p><strong>TO WHOMSOEVER IT MAY CONCERN</strong></p>

    <p>This is to certify that <strong>${params.employeeName}</strong> (Staff ID: <strong>${params.employeeNum}</strong>) has been associated with <strong>${params.companyName}</strong> as a <span class="highlight">${params.designation}</span> in the <strong>${params.department}</strong> department.</p>

    <div class="badge-box">
      <strong>Service Tenure Summary:</strong><br/>
      • <strong>Start Date:</strong> ${new Date(params.joiningDate).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}<br/>
      • <strong>Relieving Date:</strong> ${new Date(params.lastWorkingDay).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}<br/>
      • <strong>Official Position:</strong> ${params.designation}
    </div>

    <p>During their tenure with our organization, ${params.employeeName} exhibited exemplary leadership, technical proficiency, and high ethical standards. They played a vital role in executing project deliverables and collaborated effectively across team functions.</p>

    <p>We confirm that they leave us with a clean record and have completed all clearance requirements. We wish ${params.employeeName} continued professional success and prosperity in all future career endeavors.</p>
  </div>

  <div class="signature-box">
    Authorized HR Officer<br/>
    <small>${params.companyName} Corporate HR</small>
  </div>

  <div class="footer">
    This document is an official Work Experience Certificate issued by ${params.companyName}.
  </div>
</body>
</html>
  `;
